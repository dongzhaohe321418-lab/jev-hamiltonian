...
rows = [("base_rate", "Base rate"), ("stretch", "Stretch factor (stated in prompt)$^a$"), ("logistic", "Logistic (stretch, HOMO--LUMO gap)"),
...
for k, name in rows:
    ...
    for lab in ["mr", "t1", "unstable"]:
        v = e3[lab][k]; au = e3[lab]["stretch_raw"]["auroc"] if k == "stretch" else v["auroc"]
        cells += ["--" if k == "base_rate" else f(au), f(v["brier"], 3)]
