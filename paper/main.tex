...
Using black-box access to \texttt{jev-1.13.0}, PySCF ground truth and pre-registered analyses (with disclosed deviations), we find that \jev{}'s conditionals contain an approximate pairwise landscape: an Ising energy fitted to single-fact conditionals predicts held-out two-fact conditionals better than a naive baseline in all 48 cases.
The landscape is not exact.
Under a coherent null matched to \jev{}'s own noise, 41\% of pairwise coupling asymmetries are significant, and the incoherence persists when the exact joint distribution is written into the input.
...
It adds a small, significant amount of information to a two-descriptor model, ranks CCSD(T) failures about as well as the $T_1$ diagnostic, and ranks determinants for selected CI with one template that loses to iterative CIPSI.
Two open-weight language models are less coherent still and ignore the Hamiltonian.
\jev{} is best read as a fast, approximately coherent triage signal, not as a physical energy landscape.
\end{abstract}
...
\caption{\jev{} against two open-weight models (E7). \textbf{a},~Cumulative distribution of coupling asymmetry over the 480 case--pair combinations of E1; the dashed line is the 95th percentile of a coherent null with \jev{}'s noise. \textbf{b},~AUROC and Brier score on the three E3 labels. \textbf{c},~Rank correlation between probability and negative energy for random Ising models written in both sign conventions; mean $\pm$ s.e.m.\ over instances.}
\label{fig:models}
\end{figure}
